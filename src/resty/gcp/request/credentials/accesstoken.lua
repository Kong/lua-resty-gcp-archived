...
local function GetAccessTokenByWI()
  ...
  res, err = client:request_uri(auth_url, {
    headers = {
...
function AccessToken:new()
    ...
    local accessToken, authMetod  = GetAccessTokenBySA() or GetAccessTokenByWI() or GetAccessTokenHardcoded()
    if (accessToken) then
...
return setmetatable(AccessToken, {
